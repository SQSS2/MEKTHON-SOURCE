...
async def inline_on():
    ...
    await jmubot.send_message(bot, username)
    await asyncio.sleep(1)
    await jmubot.send_message(bot, "《البوت المساعد من سورس ميكثون| قناة السورس @MekThon | مطور السورس @SQSS2 》")
    await asyncio.sleep(1)
    ...
    if not log_chat:
        ...
        try:
            group_id = await jmubot(
                CreateChannelRequest(
                    title="مجموعة السجل ميكثون ",
                    about="مجموعة حفظ الأحداث والتنبيهات الخاصة بسورس ميكثون \n\n انضم لقناة السورس @MEKThon",
                    megagroup=True,
                ),
            )
