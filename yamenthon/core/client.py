...
from yamenthon.config import Var
from database import jmdB
from yamenthon.core.helper import fast_download
from yamenthon.core.logger import LOGS, TelethonLogger
...
class JmthonClient(TelegramClient):
    ...
    async def start_client(self, **kwargs):
        ...
        if self.uid in blacklisted_users:
            self.log.error(f"({me} - {self.uid}) ~ لا يمكنك أستخدام سورس أنت محظور بسبب مخالفتك سياسة أستخدام السورس @MekThon")
            sys.exit(1)
        if self._log_at:
        ...
        if custom or custom is None:
            ...
            if thumb is None:
                kwargs["thumb"] = await self._get_custom_thumb(
                    jmdB.get_key("CUSTOM_THUMBNAIL") or "resources/Yemen.jpg"
                )
