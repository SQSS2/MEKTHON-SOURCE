    ...
    def jmth(func):
        async def wrapper(event):
            ...
            if from_users and event.sender_id not in from_users:
                return await event.answer("هذه ليست لك نصب ميكثون بنفسك من @mekThon", alert=True)
            if owner and event.sender_id not in owner_and_sudos():
                return await event.answer(f"هذا هو بوت ميكثون الخاص بـ {OWNER} تابعنا @mekThonbot")
            try:
                await func(event)
            except Exception as er:
                LOGS.exception(er)
            ...
            if owner and event.sender_id not in owner_and_sudos():
                IN_BTTS = [
                    [
                        Button.url(
                            "قناة السورس",
                            url="https://t.me/MekThon",
                        ),
                        Button.url("مجموعة ميكثون", url="https://t.me/Mekthon_GROUP"),
                    ]
                ]
...
"""
                try:
                    graphLink = await mkgraph("mekthon Inline Error", MakeHtml)
                except Exception as er:
                    ...
                    await event.answer(
                        [
                            await event.builder.article(
                                ...
                                text=graphLink,
                                buttons=Button.url(
                                    "بلغ عنها", "https://t.me/Mekthon_GROUP"
                                ),
                            )
                        ]
                    )
