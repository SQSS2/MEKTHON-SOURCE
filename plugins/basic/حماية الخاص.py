...
UNAPPROVED_MSG = "**نظام حماية ميكثون الخاص بـ {ON}!**\n\n{UND}\n\nلديك {warn}/{twarn} تحذيرات"
if jmdB.get_key("PM_TEXT"):
    UNAPPROVED_MSG = (
        "**نظام حماية ميكثون الخاص بـ {ON}!**\n\n"
...
if jmdB.get_key("PMSETTING"):
        ...
        if not keym.contains(user.id) and event.text != UND:
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    if "نظام حماية ميكثون" in event.text or "**نظام حماية ميكثون" in event.text:
                        return
    ...
    @jmthon_cmd(pattern="(تشغيل|ايقاف|مسح) الارشفة$", fullsudo=True)
    async def _(e):
    ...
    @jmthon_cmd(pattern="(س|سماح)(?: |$)", fullsudo=True)
    async def approvepm(apprvpm):
        ...
        if user.id in DEVLIST:
            return await apprvpm.eor(
                "**⌔∮ هذا هو مطور سورس ميكثون هو بالأصل مسموح له بالدردشة  !**",
            )
    ...
    @jmthon_cmd(pattern="(ر|رفض)(?: |$)", fullsudo=True)
...
@jmthon_cmd(pattern="بلوك( (.*)|$)", fullsudo=True)
...
@jmthon_cmd(pattern="الغاء البلوك( (.*)|$)", fullsudo=True)
...
@jmthon_cmd(pattern="قائمة المسموحين$", owner=True)
...
async def in_pm_ans(event):
    ...
    try:
        ...
        msg_ = "**⌔∮ نظام حماية ميكثون الخاص بـ {OWNER_NAME}**"
    wrns = f"{warns}/{WARNS}"
    buttons = [
        [
        ...
        if res:
            res = [
                await event.builder.document(
                    res,
                    title="نظام حماية ميكثون الأنلاين",
                    description="~ @MEKTHON",
                    text=msg_,
                    buttons=buttons,
                    link_preview=False,
    ...
    await event.answer(res, switch_pm="• MEKTHON •", switch_pm_param="start")
