...
❃ `{i}تفعيل حماية الحساب`
    **⌔∮بعد تفعيل الامر اي شخص سيحاول التسجيل/الدخول الى حسابك التليجرام الخاص بك مباشرة سيقوم سورس ميكثون بطرده من الحساب**
...
from .. import jmthon_cmd, JmdB, jmubot


session_hashes = []

@jmthon_cmd(pattern="تفعيل حماية الحساب$")
async def save_on(e):
...
@jmthon_cmd(pattern="تعطيل حماية الحساب$")
async def save_of(e):
    if not JmdB.get_key("WORK"):
