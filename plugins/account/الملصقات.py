...
from yamenthon.helper.tools import resize_photo_sticker

from .. import LOGS, tgbot, fetch, jmdB, jmthon_cmd, inline_mention, create_quotly
...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"mekthon Stickers of {_.sender_id}"
    typee = None
    if not local:
...
@jmthon_cmd(pattern="ستيكر", manager=True)
...
@jmthon_cmd(pattern="الحزم", manager=True)
